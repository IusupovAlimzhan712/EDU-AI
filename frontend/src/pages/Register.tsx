  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        ...
        formLevel: parseInt(formData.formLevel, 10),
      });
      // Registration auto-logs the student in (server returns tokens),
      // so jump straight to the dashboard.
      if (onLogin) onLogin();
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-edu-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {errors.form && (
              <div className="p-3 rounded-lg bg-[#FEE2E2] border border-[#DC2626]/20 text-sm text-[#991B1B]">
                {errors.form}
              </div>
            )}
            {/* Full Name */}
